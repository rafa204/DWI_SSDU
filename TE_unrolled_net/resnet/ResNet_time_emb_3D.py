...
class Residual_Block3D(nn.Module):
    def __init__(self):
        ...
        # Converted to 3D Convolution
        self.conv = nn.Conv3d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=False).cuda()
        ...
        # GroupNorm works natively with 5D tensors (N, C, D, H, W)
        self.normalization = nn.GroupNorm(32, 64)
        self.tau = conf.tau_init  # conf.tau_init = default 0.1

    def forward(self, inp, emb):
        emb_out = self.emb_layers(emb)  
        # This loop automatically handles 5D inputs [Batch, Channel, Depth, Height, Width]
        # and unsqueezes emb_out to [Batch, Channels, 1, 1, 1]
...
class ResNet_time_emb3D(nn.Module):
    def __init__(self, nb_res_blocks, num_channels=2):
        super(ResNet_time_emb3D, self).__init__()
        # Converted to 3D Convolutions
        self.first_layer = nn.Conv3d(in_channels=num_channels, out_channels=64, kernel_size=3, stride=1, padding=1, bias=False)

        self.nb_res_blocks = nb_res_blocks

        res_block = []  
        for _ in range(nb_res_blocks):
            res_block += [Residual_Block3D()]        
        self.res_block = nn.Sequential(*res_block)

        self.last_layer = nn.Conv3d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, bias=False)
        self.final_conv = nn.Conv3d(in_channels=64, out_channels=num_channels, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def initialize_weights(self):
        for m in self.modules():
            # Updated initialization check for Conv3d
